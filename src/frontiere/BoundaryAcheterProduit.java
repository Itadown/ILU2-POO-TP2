	...
	public void acheterProduit(String nomAcheteur) {
		...
		if (controlAcheterProduit.vendeursProduit(produit) == null) {
			...
			for (int i = 0; i < controlAcheterProduit.vendeursProduit(produit).length; i++) {
				question.append((i+1) + " - " + controlAcheterProduit.vendeursProduit(produit)[i].getNom() + "\n");
			}
			...
			if (numVendeur > controlAcheterProduit.vendeursProduit(produit).length || numVendeur <= 0) {
				...
				if (nbProduitVendu == -1) {
					System.out.println("Je suis désolé " + nomVendeur + " mais il faut êre un habitant du village pour commercer ici.\n");
				} else if (nbProduitVendu == 0) {
					System.out.println(nomAcheteur + " veux acheter " + nbProduit + " " + produit + ", malheureusement il n'y en a plus !\n");
				} else if (nbProduitVendu < nbProduit) {
					System.out.println(nomAcheteur + " veux acheter " + nbProduit + " " + produit + ", malheureusement " + nomVendeur + " n'en a plus que " + nbProduitVendu + ". " + nomAcheteur + " achète tout le stock de " + nomVendeur +".\n");
				} else {
					System.out.println(nomAcheteur + " achète " + nbProduitVendu + " " + produit + " à " + nomVendeur + ".\n");
				}
			}
